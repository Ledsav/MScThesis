...
for i = 1:length(code_list_var)
    ...
    if isfile(direc{1}) && isfile(direc{2}) && not(isfile(file))
        ...
        save_name = strcat(save_list{1},code_list_var(i),'_ExRR_PPRR');
        saveas(gcf,save_name{1},'fig')
        saveas(gcf,save_name{1},'png')
        close all
